...
class RulesSummaryScreen extends StatelessWidget {
          ...
          SafeArea(
            child: StreamBuilder<Map<String, dynamic>>(
              stream: ChildMonitorService()
                  .watchRules(_childId, parentId: _parentId),
  ...
  Widget _buildScreenTimeCard(
      BuildContext context, Map<String, dynamic> rules) {
    final int limit = (rules['dailyLimitMinutes'] ?? 0) as int;
    final String timeStr = limit == 0
        ? 'No limit set'
        : (limit < 60
                ? '${limit}m'
                : '${limit ~/ 60}h ${limit % 60 == 0 ? '' : '${limit % 60}m'}')
            .trim();
  ...
  Widget _buildScheduleCard(BuildContext context, Map<String, dynamic> rules) {
    ...
    return GlassCard(
      ...
      child: Column(
        ...
        children: [
          _sectionHeader(
              context, 'Usage Schedule', Icons.schedule, AppColors.primary),
          SizedBox(height: 16),
          Row(
            children: [
              Icon(
  ...
  Widget _buildSafeZonesCard(BuildContext context) {
    return StreamBuilder<List<Map<String, dynamic>>>(
      stream:
          ChildMonitorService().watchGeofences(_childId, parentId: _parentId),
      builder: (context, snapshot) {
        ...
        return GlassCard(
          ...
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              _sectionHeader(context, 'Safe Zones', Icons.location_on,
  ...
  Widget _buildContentFilterCard(
    ...
    final filters = <Map<String, dynamic>>[
      ...
      },
      {
        ...
        'key': 'blockCyberbullying'
      },
      {
        'label': 'Eating Disorders',
        'icon': Icons.accessibility_new_outlined,
        'color': Colors.pinkAccent,
        'key': 'blockEatingDisorders'
      },
      // Restricted content
      {
        'label': 'Adult & Pornography',
    ...
    final activeFilters =
        filters.where((f) => rules[f['key']] == true).toList();
    final List<String> categories =
        List<String>.from(rules['customCategories'] ?? []);
    ...
    return GlassCard(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _sectionHeader(context, 'Content Filters', Icons.shield_outlined,
              Colors.orangeAccent),
          SizedBox(height: 20),
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              _sectionHeader(
                  context, 'Blocked Apps', Icons.block, Colors.redAccent),
              Spacer(),
              Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              _sectionHeader(
                  context, 'Blocked Websites', Icons.language, Colors.orange),
              Spacer(),
              Container(
                padding: EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                decoration: BoxDecoration(
  ...
  Widget _buildCustomKeywordsCard(
      BuildContext context, Map<String, dynamic> rules) {
    final List<String> keywords =
        List<String>.from(rules['customKeywords'] ?? []);
    ...
    return GlassCard(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              _sectionHeader(context, 'Custom Monitoring', Icons.manage_search,
                  AppColors.primary),
              Spacer(),
              Container(
                padding: EdgeInsets.symmetric(horizontal: 10, vertical: 4),
  ...
  Widget _buildDetailRow(
    ...
    final displayColor = active
        ? (activeColor ?? Colors.redAccent)
        : Theme.of(context).colorScheme.onSurface.withValues(alpha: 0.24);
    return Row(
      children: [
        Icon(icon,
            color: active ? displayColor : AppColors.textGray400, size: 20),
        SizedBox(width: 12),
        Expanded(
          child: Text(label,
              style: TextStyle(
