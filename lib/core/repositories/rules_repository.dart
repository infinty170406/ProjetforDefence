...
class RulesRepository {
  ...
  DocumentReference _rulesDoc(String childId) {
    ...
    return _db.collection('parents').doc(_uid).collection('children').doc(childId).collection('rules').doc('active');
  }

  /// Observe les règles en temps réel
  Stream<ChildRules> watchRules(String childId) {
    if (_uid == null || childId.isEmpty) return Stream.value(ChildRules());
    return _rulesDoc(childId).snapshots().map((snap) {
  ...
  Future<void> saveRules(String childId, ChildRules rules) async {
    ...
    await _db.collection('parents').doc(_uid).collection('children').doc(childId).update({
  ...
  Future<void> toggleWebsiteBlock(String childId, String domain, bool block) async {
    await _rulesDoc(childId).set({
  ...
  Stream<List<String>> watchInstalledApps(String childId) {
    ...
    return _db.collection('parents').doc(_uid).collection('children').doc(childId).collection('inventory').doc('apps')
        .snapshots()
        .map((snap) {
          final data = snap.data();
          return List<String>.from(data?['installedPackages'] ?? []);
        });
  }
