...
class StatsRepository {
  ...
  Future<Map<String, dynamic>> getTodayStats(String childId) async {
    ...
    if (_cache.containsKey(cacheKey)) {
      ...
      if (now.difference(cached.timestamp) < _cacheDuration) {
        if (kDebugMode) print('StatsRepository: Serving from cache for $childId');
        return cached.data;
      }
    }

    try {
      if (kDebugMode) print('StatsRepository: Fetching from Firestore for $childId');
      
      final childDoc = _db.collection('parents').doc(_uid).collection('children').doc(childId);
      ...
      final result = {
        ...
        'usedMinutes': (appData['usedMinutes'] ?? appData['totalMinutes'] ?? 0) as int,
  ...
  Stream<Map<String, dynamic>> watchTodayStats(String childId) {
    ...
    return _db.collection('parents').doc(_uid)
        ...
        .collection('alerts').doc('usage')
        .collection('apps').doc(today)
        .snapshots().map((appSnap) {
      final appData = appSnap.data() ?? {};
